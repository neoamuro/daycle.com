...
public abstract class SwipeTouchListener implements View.OnTouchListener, TouchEventHandler {
    ...
    @Nullable
    private View mLeftSwipingView;

    @Nullable
    private View mRightSwipingView;
    ...
    // 스와이프 대상 리소스 변수 추가
    private int itemViewResid = R.id.itemContent;

    // 왼쪽으로 스와이프 했을때의 배경
    private int itemLeftSwiteResid = R.id.itemLeftSwite;

    // 오른쪽으로 스와이프 했을때의 배경
    private int itemRightSwiteResid = R.id.itemRightSwite;

    /**
     * The {@link com.daycle.daycleapp.custom.swipelistview.itemmanipulation.swipedismiss.DismissableManager} which decides
     * whether or not a list item can be swiped.
     */
    @Nullable
    private com.daycle.daycleapp.custom.swipelistview.itemmanipulation.swipedismiss.DismissableManager mDismissableManager;
    ...
    private boolean mflingToRight = false;
    ...
    /**
     * Sets the {@link com.daycle.daycleapp.custom.swipelistview.itemmanipulation.swipedismiss.DismissableManager} to specify which views can or cannot be swiped.
     *
     * @param dismissableManager {@code null} for no restrictions.
     */
    public void setDismissableManager(@Nullable final com.daycle.daycleapp.custom.swipelistview.itemmanipulation.swipedismiss.DismissableManager dismissableManager) {
    ...
    // 오른쪽으로 플링했는지 값 가져오기
    public boolean getFlingToRight(){
        return mflingToRight;
    }
    ...
    private boolean handleDownEvent(@Nullable final View view, @NonNull final MotionEvent motionEvent) {
        ...
        mCurrentView = downView; // 터치다운을 한 리스트의 아이템 뷰 (한마디로 아이템 루트 레이아웃)
        mSwipingView = getSwipeView(downView); // 스와이핑할 아이템 가져옴!!
        mLeftSwipingView = getLeftSwipeView(downView); // 왼쪽 스와이핑 배경 아이템 가져옴!!
        mRightSwipingView = getRightSwipeView(downView); // 오른쪽 스와이핑 아이템 가져옴!!
    ...
    private boolean handleMoveEvent(@Nullable final View view, @NonNull final MotionEvent motionEvent) {
        ...
        // 터치 움직임의 속도 측정을 위해 벨로시티에 이벤트 추가
        mVelocityTracker.addMovement(motionEvent);
    ...
    private void flingView(@NonNull final View view, final int position, final boolean flingToRight) {
        ...
        mflingToRight = flingToRight; // 오른쪽으로 플링 했는지 체크
    ...
    protected View getSwipeView(@NonNull final View view) {
        ...
        // 스와이핑할 리소스아이디를 지정했다면 그 아이디로 가져오기
        if(itemViewResid > 0)
            return view.findViewById(itemViewResid);

        return view;
    }

    @NonNull
    protected View getLeftSwipeView(@NonNull final View view) {

        // 스와이핑할 리소스아이디를 지정했다면 그 아이디로 가져오기
        if(itemLeftSwiteResid > 0)
            return view.findViewById(itemLeftSwiteResid);

        return view;
    }

    @NonNull
    protected View getRightSwipeView(@NonNull final View view) {

        // 스와이핑할 리소스아이디를 지정했다면 그 아이디로 가져오기
        if(itemRightSwiteResid > 0)
            return view.findViewById(itemRightSwiteResid);
